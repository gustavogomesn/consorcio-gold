...
function LoanPayment() {
    ...
    async function handleLoanPaymentSubmit(e){
        ...
        try {
            const response = await fetch("http://localhost:8000/loan-payment-temporarily/", {
                method: "POST",
                body: formData,
            });
        } catch (error) {
    ...
    return (
    <>
        <button onClick={() => setIsOpen(true)} className='dropdown-item' >Pagamento de empréstimo</button>
                    ...
                    <form className='d-flex flex-column gap-2'>
                        <div className="form-group">
                            ...
                            <select className='form-control' id='borrower-input' name='loan_id' onChange={handleChangeOfBorrower}>
                                ...
                                {loans.map((loan) => (
                                    <option value={loan.id} data-loan-value={loan.value}>{loan.borrower}</option>
                                ))}
                            </select>
                        </div>
                        <div className="form-group">
